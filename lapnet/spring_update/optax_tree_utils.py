...
def tree_sub(tree_x: Any, tree_y: Any) -> Any:
    ...
    """
    return jax.tree_util.tree_map(operator.sub, tree_x, tree_y)
...
def tree_mul(tree_x: Any, tree_y: Any) -> Any:
    ...
    """
    return jax.tree_util.tree_map(operator.mul, tree_x, tree_y)
...
def tree_div(tree_x: Any, tree_y: Any) -> Any:
    ...
    """
    return jax.tree_util.tree_map(operator.truediv, tree_x, tree_y)
...
def tree_vdot(tree_x: Any, tree_y: Any) -> chex.Numeric:
    ...
    return jax.tree_util.tree_reduce(operator.add, vdots, initializer=0)
...
def tree_max(tree: Any) -> chex.Numeric:
    ...
    return jax.tree_util.tree_reduce(jnp.maximum, maxes, initializer=jnp.array(-jnp.inf))
...
def tree_l1_norm(tree: Any) -> chex.Numeric:
    ...
    Returns:
      a scalar value.
    """
    abs_tree = jax.tree_util.tree_map(jnp.abs, tree)
...
def tree_ones_like(
    ...
    """
    return jax.tree_util.tree_map(lambda x: jnp.ones_like(x, dtype=dtype), tree)


def tree_full_like(
        tree: Any,
        fill_value: chex.Array,
        dtype: Optional[chex.ArrayDType] = None,
) -> Any:
...
def tree_clip(
        tree: Any,
        min_value: Optional[chex.Array],
        max_value: Optional[chex.Array],
) -> Any:
    ...
    .. versionadded:: 0.2.3
    """
    return jax.tree_util.tree_map(lambda g: jnp.clip(g, min_value, max_value), tree)
